...
def test_sanity(server_port):
    with Server("/home/maayan123/os3_wet/server", server_port, 1, 1) as server:
        sleep(0.1)
        with FuturesSession() as session1:
            future1 = session1.get(f"http://localhost:{server_port}/output.cgi?1") #sending a request for output.cgi
            sleep(0.1)
            with FuturesSession() as session2:
                future2 = session2.get(f"http://localhost:{server_port}/output.cgi?1")
                response = future2.result() # the server's answer
                expected_headers = generate_dynamic_headers(123, 2, 0, 2,0) #the last argument is todo
                expected = DYNAMIC_OUTPUT_CONTENT.format(
                    seconds="1.0")
                validate_response_full(response, expected_headers, expected) # comparing expected against response
            response = future1.result()
            expected_headers = generate_dynamic_headers(123, 1, 0, 1, 0)#the last argument is todo
            expected = DYNAMIC_OUTPUT_CONTENT.format(
                seconds="1.0")
...
def test_load(threads, queue, amount, dispatches, server_port):
    with Server("/home/maayan123/os3_wet/server", server_port, threads, queue) as server:
        sleep(0.1)
        ...
        for i in range(amount):
            ...
            expected_headers = generate_dynamic_headers(123, (i // threads) + 1, 0, (i // threads) + 1, 0) #the last argument is todo
