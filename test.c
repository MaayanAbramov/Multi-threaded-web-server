...
int no_skip_log = 0; // global variable to determine if this program should skip test or not.
...
void parse_args(int argc, char* argv[]){
    if(argc < 2 ){
        printf("usage: ./test <no_skip_log>, no_skip_log should be 1 if you dont want to skip, should be 0 if you want to skip!\n");
        exit(1);
    }
    int status = atoi(argv[1]);
    if(status != 0 && status!=1){
        printf("no_skip_log invalid argument, please input 0 or 1 only.\n");
        exit(1);
    }
    no_skip_log = status;
    return;
